...
const getUrlNew = async (req, res) => {
  ...
  while (!purl && count < 10) {
    ...
    const result = await request({
      ...
      // https://ws.stream.qqmusic.qq.com/C4000030p9ps1GC3FZ.m4a?
      // guid=3543223420&
      // vkey=CD6151EB7EDF952357827F318E264DF2150F46E8AC565031EA4AD4D8336CF06E4F6C82B78219DA94EE05270E01EA1B88F924C8F8BFCF0935&
      // uin=2404&
      // fromtag=66
  ...
  if (Number(isRedirect)) {
    ...
    return res.redirect(`https://ws.stream.qqmusic.qq.com/${purl}`);
  }

  cacheData = {
    // data: `http://122.226.161.16/amobile.music.tc.qq.com/${purl}`,
    data: `https://ws.stream.qqmusic.qq.com/${purl}`,
    result: 100,
  }
  res.send(cacheData);
  cache.set(cacheKey, cacheData);
...
router.get('/urls', getUrlNew);

router.post('/urls', getUrlNew);
